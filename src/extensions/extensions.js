...
const ingredients = ['flour', 'sugar', 'milk']
...
function calculateQuantities(ingredients, layers) {
  ...
  if (ingredients.includes('sugar')) {
    quantities.sugar = sugarPerLayer * layers
  }

  if (ingredients.includes('eggs')) {
    quantities.eggs = eggsPerLayer * layers
  }
  return quantities
}
...
function improveRecipe(ingredients, portions) {
  ...
  for (const key in ingredients) {
    updatedIngredients[key] = ingredients[key] * portions
  }
